...
//			mainMotionManager.setMotionDebugMode(true);
	...
	public void load(GL10 gl,String modelSettingPath) throws Exception
	{
		...
		if (modelSetting.getLayout(layout) )
		{
			...
			if (layout.get("center_x")!=null)modelMatrix.centerX(x = layout.get("center_x"));
			if (layout.get("center_y")!=null)modelMatrix.centerY(y = layout.get("center_y"));
	...
	public void startInOrderMotion(String name,int n,int priority){
		int max = modelSetting.getMotionNum(name);
		int no = n % max;
		Log.d("live2d_motionNo.",""+no);
		startMotion(name,no,priority);
	}
	...
	public void setX(float x){
		modelMatrix.centerX(x);
		//modelMatrix.setCenterPosition(x,this.y);
		this.x = x;
	}
	public void setY(float y){
		modelMatrix.setCenterPosition(this.x,y);
		this.y = y;
	}
